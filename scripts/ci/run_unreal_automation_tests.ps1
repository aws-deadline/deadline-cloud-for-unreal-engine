...
try {
    ...
    $requiredTestPrefixes = @(
        "DeadlineCloud.Offline.DeadlineCloudMRQJobUI.MRQJobUI"
    )
    foreach ($requiredPrefix in $requiredTestPrefixes) {
        if (-not ($startedPaths | Where-Object { $_.StartsWith($requiredPrefix) })) {
            throw "Required UI automation test did not run: $requiredPrefix"
        }
    }
    if (-not $queueDrained) {
