...
class DynamicChunkingHelper:
    ...
    - RangeConstraint validation (CONTIGUOUS/NONCONTIGUOUS)
    ...
    def _validate_range_constraint(range_constraint: str) -> None:
        """
        Validate that RangeConstraint value is either CONTIGUOUS or NONCONTIGUOUS.

        This validation is used for the TASK_CHUNKING extension to ensure that
        the RangeConstraint parameter has a valid value before job submission.

        :param range_constraint: The RangeConstraint value to validate
        :type range_constraint: str

        :raises SubmitterInputValidationError: If value is not CONTIGUOUS or NONCONTIGUOUS
        """
        valid_values = {"CONTIGUOUS", "NONCONTIGUOUS"}
        if range_constraint not in valid_values:
            raise SubmitterInputValidationError(
                f'Invalid RangeConstraint value "{range_constraint}". '
                f"RangeConstraint must be one of: {', '.join(sorted(valid_values))}."
            )
    ...
    def validate_frames_parameter(frames: str) -> None:
        ...
        - "0": Special value meaning render all frames (uses level sequence playback range)
        - Single non-negative integer: "5", "100"
        - Range: "1-100", "0-50"
        - Range with step: "1-100:2", "0-100:5"
        - Comma-separated list (IntRangeList): "1-10,15,20-30", "1,2,3"
        - Mixed formats: "1-10:2,15,20-30"

        Note: Empty frames is not valid. Use "0" to render all frames.
        Note: Negative frame numbers are not supported.
        ...
        if not frames or not frames.strip():
            raise SubmitterInputValidationError(
                "Frames parameter is required. Use '0' to render all frames, "
                "or specify a frame range (e.g., '1-100', '1,5,10-20:2')."
            )

        frames = frames.strip()

        # "0" is a special value meaning render all frames
        if frames == "0":
            return
        ...
        # - Single non-negative integer: "5", "0", "100"
        # - Range: "1-100", "0-50"
        # - Range with step: "1-100:2", "0-100:5" (step only valid with range)
        ...
        #
        # Pattern breakdown:
        # - Each element can be: single integer OR range (with optional step)
        # - Single integer: \d+ (one or more digits, no negative)
        # - Range with optional step: \d+-\d+(?::\d+)? (start-end with optional step)
        # - Step is only valid when there's a range (not for single integers)
        # - Elements separated by commas
        #
        # The pattern uses alternation: (range_with_optional_step | single_integer)
        single_int = r"\d+"
        ...
        if not frames_validation_pattern.match(frames):
            raise SubmitterInputValidationError(
                ...
                f"Valid formats include: non-negative integers ('5'), ranges ('1-100'), "
