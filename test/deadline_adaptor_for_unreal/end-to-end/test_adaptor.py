...
unreal_adaptor_executable = str(Path(current_python_executable).parent / "UnrealAdaptor.exe").replace('\\', '/')
...
def rebuild_adaptor():
    ...
    os.system(
        f'{current_python_executable} -m pip uninstall deadline-cloud-for-unreal --yes'
    )
...
class TestUnrealAdaptor:
    ...
    @pytest.mark.parametrize(
        'init_data, run_data',
        [
            (init_data(), run_data_render()),
            (init_data(), run_data_custom())
        ]
    )
