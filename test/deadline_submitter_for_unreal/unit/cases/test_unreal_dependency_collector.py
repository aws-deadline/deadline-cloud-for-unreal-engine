...
class TestUnrealDependencyCollector(unittest.TestCase):
    ...
    def test_dependency_filter_in_game_folder(self):
        for case in [
            ...
            self.assertEqual(
                common.DependencyFilters.dependency_in_game_folder(case[0]),
                case[1]
            )

    def test_os_path_from_unreal_path(self):
        for case in [
            ("/Game/Test/TestLevelSequence", True, f"{UNREAL_PROJECT_DIRECTORY}/Content/Test/TestLevelSequence.uasset"),
            ("/Game/Test/TestLevel", True, f"{UNREAL_PROJECT_DIRECTORY}/Content/Test/TestLevel.umap"),
            ...
            self.assertEqual(
                common.os_path_from_unreal_path(case[0], with_ext=case[1]),
                case[2]
            )

    def test_os_abs_from_relative(self):
        for case in [
            ('RelativePath/WithoutDiskLetter/test.py', f'{UNREAL_PROJECT_DIRECTORY}/RelativePath/WithoutDiskLetter/test.py'),
            ('C:/Users', 'C:/Users')
        ]:
            self.assertEqual(
                common.os_abs_from_relative(case[0]),
                case[1]
            )
    ...
    def test_dependency_collector(self):
        ...
        self.assertEqual(
            dependencies, UNREAL_ASSET_DEPENDENCIES_PATHS
        )
        self.assertEqual(
            len(dependencies), len(UNREAL_ASSET_DEPENDENCIES_PATHS)
        )
