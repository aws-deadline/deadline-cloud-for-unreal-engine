...
def f_job_template_default() -> dict:
    return {
        ...
        'parameterDefinitions': [
            {'name': 'ParamA', 'type': 'PATH', 'objectType': 'FILE', 'dataFlow': 'IN', 'default': 'path/to/file'},
    ...
    return {
        ...
        'parameterSpace': {
            'taskParameterDefinitions': [
                {'name': 'ParamB', 'type': 'PATH', 'range': ['path/to/file']},
                {'name': 'ParamA', 'type': 'STRING', 'range': ['foo']},
        ...
        'script': {
            ...
            'actions': {
                'onRun': {'command': '{{Task.File.Run}}', 'cancelation': {'mode': 'NOTIFY_THEN_TERMINATE'}}
            }
        }
    }
...
def f_environment_template_default() -> dict:
    return {
        ...
        'script': {
            ...
            'actions': {
                'onEnter': {'command': '{{Env.File.Init}}', 'cancelation': {'mode': 'NOTIFY_THEN_TERMINATE'}},
                'onExit': {'command': '{{Env.File.Exit}}', 'cancelation': {'mode': 'NOTIFY_THEN_TERMINATE'}}
            }
        }
    }
