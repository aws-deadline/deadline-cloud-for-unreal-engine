...
"""
E2E tests for adaptor fallback scenarios (T3, T4).

Uses a single UE job submission to generate a job bundle on disk, then modifies
and resubmits it twice via the API to test:
- T3: Conda fallback when no bundle attached (mimics old submitter)
- T4: No bundle + no conda → job fails with clear error
"""
...
def test_adaptor_fallback_scenarios(
    ...
    1. T3: With unrealengine-openjd in CondaPackages, no bundle → conda fallback
    2. T4: No unrealengine-openjd, no bundle → job fails
    """
    ...
    # --- T3: Conda fallback (mimics old submitter) ---
    logger.info("T3: Testing conda fallback (no bundle, with unrealengine-openjd)")
    ...
    assert success, f"T3: Job did not succeed: {message}"
    ...
    assert conda_used, "T3: Expected conda fallback but log message not found"
    logger.info(f"T3 PASSED: Job {job_id} used conda fallback")

    # --- T4: No bundle, no conda → clear error ---
    logger.info("T4: Testing no adaptor available (no bundle, no conda)")
    ...
    assert status == "FAILED", f"T4: Expected FAILED but got: {status}"
    logger.info(f"T4 PASSED: Job {job_id} failed when no adaptor available")
